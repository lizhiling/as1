...
function DormitorySummary() {
    ...
    $("#dormitory").show();
    return {'maleCount': maleCount, 'femaleCount': femaleCount, 'maleEmps': maleEmps, 'femaleEmps': femaleEmps};
}
...
function PartySummary() {
    ...
    $("#party").show();
    // return {'m20':m20,'m40':m40,'m60':m60,'f20':f20,'f40':f40,'f60':f60};
}
...
function generateForm(data) {
    emptyForm();
    var themeId = parseInt(localStorage.getItem('themeId'));
    if (themeId == 1) {
        WelfareForm(data);
    } else if (themeId == 2) {
        TrainingForm(data);
    ...
    console.log("haha");
    var form = $("#funds-apply-form");
    form.modal('show');
}

function WelfareForm(data) {
    var emps = JSON.parse(localStorage.getItem('emps'));
    var totalAmount = 0;
    for (var i = 0; i < emps.length; i++) {
        var emp = emps[i];
...
function DormitoryForm(data) {
    ...
    var pplPerRoom = 8;
    $("#tamount").text(amount * (Math.ceil(maleCount / pplPerRoom) + Math.ceil(femaleCount / pplPerRoom)));
    $("#date").text(localStorage.getItem('date'));
    $("#purpose").text("Arrange Dormitory");
    var temp = '';
    for (var i = 0; i < data.maleEmps.length; i++) {
        var maleEmp = data.maleEmps[i];
        temp += maleEmp + ", ";
        if ((i + 1) % pplPerRoom == 0 || i == data.maleEmps.length - 1) {
            appendEmpAmount(temp.substr(0, temp.length - 2), amount);
            temp = '';
        }
    }
...
function WPForm(data) {
    ...
    $("#tamount").text(totalAmount);
    $("#date").text(localStorage.getItem('date'));
    $("#purpose").text("Apply Visa");
}

function appendEmpAmount(empId, amount) {
    //append each emp id and amount pairs to table.
    var reportTable = $("#funds-apply-form table");
    var row = $("<tr class='active emp-amount'><td>" + empId + "</td> <td>" + amount + "</td></tr>")
